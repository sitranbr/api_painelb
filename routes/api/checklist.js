    ...
    if (err instanceof multer.MulterError) {
      ...
      try {
        ...
        if (Array.isArray(photoData) && photoData.length) {
          var mainPath = req.protocol + '://' + req.get('host') + "/uploads/checklist/" + id + "/";
          let photoResult = await photoChecklistService.saveCheckListPhotos(photoData, mainPath);
        }
...
router.get("/:id", async (req, res) => {
  try {
    checklistService.getChecklistById(req.params.id)
      .then(result => {
        if (result.length) {
          res.json(result[0]);
        } else {
  ...
  uploadImage(req, res, async function (err) {
    ...
    if (err instanceof multer.MulterError) {
      ...
      try {
        ...
        let checkResult = await checklistService.updateChecklist(checklistId, checklist);
        let photoDeleteResult = await photoChecklistService.deletePhotoByChecklistId(checklistId);
        if (Array.isArray(photoData) && photoData.length) {
          var mainPath = req.protocol + '://' + req.get('host') + "/uploads/checklist/" + id + "/";
          let photoResult = await photoChecklistService.saveCheckListPhotos(photoData, mainPath);
        }
